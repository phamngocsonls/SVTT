...
int ouput_append_latency(rados_ioctx_t io, const char *data, size_t len, size_t num_writes){
    ...
    for (size_t i = 0; i < num_writes; ++i){
        ...
        timersub(&time[i].commit_end, &time[i].start, &commit_latency);
...
int main(int argc, const char ** argv){
	...
	err = rados_conf_read_file(cluster, "~/Documents/Ebook/Clanguage/learning/ceph.conf");
	if (err <0) {
    ...
    // Tạo IO context
    err = rados_ioctx_create(cluster, "rbd", &io);
	if(err < 0) {
		fprintf(stderr, "%s: Cannot open rados pool %s: %s\n", argv[0], "rbd", strerror(-err));
